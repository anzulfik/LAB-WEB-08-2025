...
@section('content')
    <h1 class="text-3xl font-bold text-gray-100 mb-6 tracking-wider">UBAH DATA SPESIMEN</h1>

    <div class="bg-black/30 backdrop-blur-md rounded-lg border border-blue-500/30 overflow-hidden shadow-lg">
        <form action="{{ route('ikan.update', $fish) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="p-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6">
                    ...
                    <div>
                        ...
                        <input type="number" name="base_weight_min" id="base_weight_min" value="{{ old('base_weight_min', $fish->base_weight_min) }}" required step="0.01"
                               class="block w-full bg-gray-900/50 border border-gray-600 rounded-md focus:border-blue-500 focus:ring-blue-500 sm:text-sm text-gray-200 placeholder-gray-500 px-3 py-2">
                    </div>
                    <div>
                        <label for="base_weight_max" class="block text-sm font-medium text-gray-400 mb-1">Berat Max. (kg)</label>
                        <input type="number" name="base_weight_max" id="base_weight_max" value="{{ old('base_weight_max', $fish->base_weight_max) }}" required step="0.01"
                               class="block w-full bg-gray-900/50 border border-gray-600 rounded-md focus:border-blue-500 focus:ring-blue-500 sm:text-sm text-gray-200 placeholder-gray-500 px-3 py-2">
                    </div>
                    <div>
                        <label for="sell_price_per_kg" class="block text-sm font-medium text-gray-400 mb-1">Nilai (Kredit/kg)</label>
                        <input type="number" name="sell_price_per_kg" id="sell_price_per_kg" value="{{ old('sell_price_per_kg', $fish->sell_price_per_kg) }}" required step="1"
                               class="block w-full bg-gray-900/50 border border-gray-600 rounded-md focus:border-blue-500 focus:ring-blue-500 sm:text-sm text-gray-200 placeholder-gray-500 px-3 py-2">
                    </div>
                    <div>
                        <label for="catch_probability" class="block text-sm font-medium text-gray-400 mb-1">Probabilitas Tangkap (%)</label>
                        <input type="number" name="catch_probability" id="catch_probability" value="{{ old('catch_probability', $fish->catch_probability) }}" required step="0.01" min="0.01" max="100"
                               class="block w-full bg-gray-900/50 border border-gray-600 rounded-md focus:border-blue-500 focus:ring-blue-500 sm:text-sm text-gray-200 placeholder-gray-500 px-3 py-2">
                    </div>
                    <div class="md:col-span-2">
                        <label for="description" class="block text-sm font-medium text-gray-400 mb-1">Catatan Data (Opsional)</label>
                        <textarea name="description" id="description" rows="4"
                                  class="block w-full bg-gray-900/50 border border-gray-600 rounded-md focus:border-blue-500 focus:ring-blue-500 sm:text-sm text-gray-200 placeholder-gray-500 px-3 py-2">{{ old('description', $fish->description) }}</textarea>
                    </div>
                </div>
            </div>
