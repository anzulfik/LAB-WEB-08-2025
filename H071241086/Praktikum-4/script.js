...
function gambarAman(src, className = 'card-img') {
  ...
  img.onerror = function () {
    this.onerror = null; // supaya tidak terulang
...
//Lapisan Wild (pilih warna kartu)
function tampilLapisanWild() {
  wildOverlay.style.display = 'flex'; //ditengah layar
}

function sembunyiLapisanWild() {
  wildOverlay.style.display = 'none'; //menghilangkan tampilan
}

function pilihWarnaWild(color) {
  const played = keadaan.discard[keadaan.discard.length - 1]; // kartu terakhir ditumpukan discard
  if (played) {
    played.color = color;
...
function tampilNotifikasi(text, duration = 1800, bg = '#f9f9f9ff') {
  if (!notif) return;
  notif.textContent = text;
  notif.style.background = bg;
  notif.style.display = 'block'; //menyembukikan notifikasi
...
function aturWarnaAktif(color) { //setelah wild
...
function ambilKartu() {
  if (keadaan.deck.length === 0) { // kartu baru habis
    if (keadaan.discard.length <= 1) return null; //kasih sisa 1

    const top = keadaan.discard.pop();
    keadaan.deck = keadaan.discard.splice(0);
    acakKartu(keadaan.deck);
    keadaan.discard = [top]; //pindahkan kartu teratas
  }
  return keadaan.deck.pop();
}

//saat ingin membuang kartu di discard
...
function pemainPunyaKartuLainBukanWild() {
  const top = kartuTeratas();
  return keadaan.player.hand.some(c => { //mengembalikan kartu
    if (!c) return false;
...
function renderTanganPemain() {
  playerHandEl.innerHTML = ''; //mengosongkan tampilan pemain
  ...
  if (keadaan.player.hand.length === 1) { //tombol UNO
...
function renderTanganBot() {
  ...
  botCountEl.textContent = keadaan.bot.hand.length; //memperbarui teks jumlah karu
}

//kartu paling atas discard
...
function perbaruiKeadaanDeck() {
  ...
  if (hasPlayable || keadaan.playerDrew) {
    ...
    deckEl.classList.remove('deck-disabled'); //jika kartu tidak ada yang cocok
...
function perbaruiTampilan() {
  ...
  if (keadaan.turn === 'player' && keadaan.playerDrew) { //button lewati giliran
  ...
  if (keadaan.bot.hand.length === 1 && !keadaan.bot.saidUNO && !keadaan.roundOver) { //button Panggil UNO
...
function mulaiRondeBaru() {
  ...
  keadaan.bet = bet; //nilai taruhan pemain
  ...
  if (first) keadaan.discard.push(first); //ambil kartu dari deck
  if (first && first.color) aturWarnaAktif(first.color);

 // giliran pemain atau bot jika dironde sebelumnya menang
  ...
  if (keadaan.turn === 'bot') setTimeout(giliranBot, 500);
...
function mainkanKartu(who, idx) {
  ...
  keadaan.turn = who === 'player' ? 'bot' : 'player'; //mengatur giliran berikutnya
...
function giliranBot() {
  ...
  for (let i = 0; i < hand.length; i++) {
    if (isCardPlayableAgainstTop(hand[i])) { //mengecek kartu yang cocok
      mainkanKartu('bot', i);
      return;
    }
  }
  ...
  const last = hand[hand.length - 1]; //kartu terakhir di bot
...
deckEl.addEventListener('click', () => {
  if (keadaan.turn !== 'player' || keadaan.playerDrew) return; // kalau buka pemain tidak bisa menarik kartu
  const c = ambilKartu();
  if (c) {
